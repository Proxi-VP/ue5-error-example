...
def typeHintTextClassGenerator(widgets: list[str], windowType: WindowType, forceImportBaseClass: bool=False) -> list[str]:
    ...
    if windowType == WindowType.dialog:
        widgets.insert(2, '    from PySide6.QtWidgets import QDialog\n' if forceImportBaseClass else '')
        ...
        widgets.insert(2, '    from PySide6.QtWidgets import QMainWindow\n' if forceImportBaseClass else '')
        widgets.insert(3, '    class _TypeHint:\n')
        widgets.insert(4, '        """Auto-generated type hinting class"""\n\n')
...
def main():
    ...
    uiDir = os.path.join(pythonBaseDir, 'proxi/ui').replace('\\', '/')
